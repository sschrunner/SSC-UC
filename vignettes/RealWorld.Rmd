...
```{r evaluate1, fig.height = 6, fig.width = 6, fig.align = 'center'}
library(caret)
library(knitr)
library(ggplot2)
...
kable(cm$table[paste0(sort(unique(pred))),paste0(sort(unique(test$y))), drop = F])
mean(cm$byClass[,"F1"], na.rm = TRUE)

df <- data.frame(
  value = as.vector(cm$table[paste0(sort(unique(pred))),paste0(sort(unique(test$y)))]),
  predicted = rep(sort(unique(pred)), each = length(unique(test$y))),
  true = rep(sort(unique(test$y)), length(unique(pred)))
)
...
```{r plot1, fig.height = 6, fig.width = 12, fig.align = 'center'}
library(ggplot2)
library(latex2exp)
