...
Each class $i$ is represented by a bivariate Gaussian distribution with mean vector $\mu = (\mu_1^{(i)}, \mu_2^{(i)})$ and covariance matrix $$\Sigma_i = \left(\begin{array}{cc}\sigma_{1,1}^{(i)} & \sigma_{1,2}^{(i)} \\ \sigma_{1,2}^{(i)} & \sigma_{2,2}^{(i)}\end{array}\right)$$. The known classes are 0-3.
